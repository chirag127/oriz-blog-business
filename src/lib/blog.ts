...
/**
 * Content verticals — the five founder/freelancer "accounts" this ledger
 * covers. Keys are kept (dev/finance/culture/health/personal) so the shared
 * --v-* posting-mark colours resolve, but the labels + matching are business:
 *   dev → Startups · finance → Money · culture → Freelance · health → Growth ·
 *   personal → Notes.
 */
...
/** Map a raw category string → one of the five accounts. */
export function verticalFor(category: string): Vertical {
  const c = category.toLowerCase()
  if (/(growth|marketing|sales|acquisition|customer|scale|traction|funnel)/.test(c)) return 'health'
  if (/(money|finance|pricing|revenue|cash|invest|tax|profit|margin|fundrais|cost)/.test(c))
    return 'finance'
  if (/(startup|founder|idea|product|mvp|validat|launch|build|company|venture)/.test(c)) return 'dev'
  if (/(freelance|solo|client|contract|consult|agency|rate|gig|self-employ)/.test(c)) return 'culture'
  return 'personal'
}
